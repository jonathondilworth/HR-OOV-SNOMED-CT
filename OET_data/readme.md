...
|   |   |   |   train.jsonl
|   |   mention-level-(concept-placement) # Mention-level data
|   |   |   train.jsonl
|   |   |   valid.jsonl
|   |   |   test-in-KB.jsonl
|   |   |   valid-NIL.jsonl
|   |   |   test-NIL.jsonl
|   |   |   test-NIL-all.jsonl # (the combination of valid-NIL.jsonl and test-NIL.jsonl)
...
|   |   |   |   test-NIL.jsonl
|   |   |   |   test-NIL-all.jsonl # the combination of valid-NIL.jsonl and test-NIL.jsonl
|   |   |   |   test-NIL-all-complex.jsonl # mentions with complex edges in test-NIL-all.jsonl
...
p.s. Note that we renamed the sub-folder names after running the scripts for easier understanding.
     The original sub-folder names are below:     
        st21pv_syn_attr-all-complexEdge-filt        -> mention-level-(out-of-KB-discovery)/syn_attr
        st21pv_syn_full-all-complexEdge-filt        -> mention-level-(out-of-KB-discovery)/syn_full
        st21pv_syn_attr-all-complexEdge-unsup       -> mention-level-(concept-placement)
